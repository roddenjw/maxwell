...
export default function EventList({ manuscriptId }: EventListProps) {
  ...
  const handleExtractTimeline = async () => {
    console.log('🔍 Extract Timeline clicked');
    ...
    console.log(`📊 Events: ${totalCount} total, ${autoGeneratedCount} auto-generated, ${unmarkedCount} unmarked`);
    ...
    if (totalCount > 0) {
      ...
      if (unmarkedCount > 0) {
        const choice = confirm(
          `Found ${totalCount} existing event(s):\n` +
          `• ${autoGeneratedCount} auto-generated\n` +
          `• ${unmarkedCount} from before tracking was added\n\n` +
          `Options:\n` +
          `OK = Clear ALL events and re-extract fresh\n` +
          `Cancel = Keep existing events and only add new ones\n\n` +
          `(User-created events will be protected in future extractions)`
        );

        if (choice) {
          // User wants to clear all
          shouldClearEvents = true;
        }
      } else if (autoGeneratedCount > 0) {
        // All events are marked as auto-generated
        const choice = confirm(
          `Found ${autoGeneratedCount} auto-generated event(s) from previous extractions.\n\n` +
          `Click OK to clear them and re-extract fresh.\n` +
          `Click Cancel to keep them and only add new events.`
        );
        shouldClearEvents = choice;
      }
    } else {
    ...
    console.log(`🚀 Starting extraction, shouldClearEvents: ${shouldClearEvents}`);
    ...
    try {
      ...
      setAnalysisProgress('');
      alert(`Timeline extraction complete! Analyzed ${processedCount} chapter(s). Check the Visual tab to see your timeline.`);
    } catch (err) {
